...
Task { @MainActor in
    do {
        ...
        let picker = CodexChatModel(reference: reference, directory: directory, memory: memory, agents: store)
        picker.chooseAgent(.preset(.smartwork), store: store)
        check(picker.agent.kind == .smartwork && store.selectedID == "smartwork", "Picker updates the settings default")
        check(AgentConfigurationStore(defaults: defaults).selectedID == "smartwork", "Picker choice persists after restart")
        picker.messages = [user]
        picker.chooseAgent(.preset(.claude), store: store)
        check(picker.messages.isEmpty && store.selectedID == "claude", "Switching from history starts a conversation and remembers the provider")
        let next = CodexChatModel(reference: reference, directory: directory, memory: memory, agents: AgentConfigurationStore(defaults: defaults))
